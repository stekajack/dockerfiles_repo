...
I use a wrapper script in this repo `dh` (Python standard library only) to manage images:
```bash
./dh build -f dockerfile_src  user/image_name:tag
./dh push user/image_name:tag --dry-run # preview the generated README
./dh push user/image_name:tag # docker push + update the DockerHub README
```
The README of each DockerHub repository is generated by `dh push` from [README.template.md](README.template.md): one section per tag, listing the branch and commit of each of my packages in the image (espresso, pressomancy, pressomancy-toolbox, espressoSq), the ESPResSo CMake flags, `espressomd.features()` and a link to the Dockerfile at the commit it was built from. Run `./dh -h` and `./dh push -h` for the details. The plain docker commands below work just as well; you only lose the generated README.
